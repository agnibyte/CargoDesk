...
export default function ContactsPreviewList({ contacts }) {
  ...
  return (
    <div className="mt-6">
      <div className="flex items-center justify-between mb-3">
        ...
        <button className="px-6 py-2 bg-violet-700 text-white rounded hover:bg-violet-950 transition duration-300">
          Import All
        </button>
      </div>
        ...
        {contacts.map((c, i) => {
          ...
          return (
            ...
            >
              {!isConfirm ? (
                <>
                  <div className="w-[85%] lg:w-[90%] p-5 bg-white text-gray-800 flex items-center justify-between">
                    <div className="text-md font-semibold">{c.name}</div>
                    <div className="text-md font-semibold">{c.contactNo}</div>
                  </div>
                  {!isConfirm && (
                    <button
